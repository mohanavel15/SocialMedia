...
func GetFeed(user User, limit int64, offset int64) []Post {
	...
	cur, err := postCollection.Find(context.TODO(), bson.M{"author_id": bson.M{"$in": user.Following}}, options.Find().SetSort(bson.M{"created_at": -1}).SetLimit(limit).SetSkip(offset))
	if err != nil {
		return posts
	}
